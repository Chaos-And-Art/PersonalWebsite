...
@Component({
  selector: 'app-obsidian-dashboard',
  templateUrl: './obsidian-dashboard.component.html',
  styleUrls: ['./obsidian-dashboard.component.scss']
})
export class ObsidianDashboardComponent {
  ...
  constructor(public dialog: MatDialog, private themeService: ThemingService, private router: Router, private elementRef: ElementRef, private authService: AuthService) {
  ...
  ngAfterViewInit() {
    ...
    const myThemeSlidersOn = this.elementRef.nativeElement.querySelectorAll('.mdc-switch__icon--on');
    for (let i = 0; i < myThemeSlidersOn.length; i++) {
      myThemeSlidersOn[i].firstChild.setAttribute('d', moon);
    }

    const myThemeSlidersOff = this.elementRef.nativeElement.querySelectorAll('.mdc-switch__icon--off');
    for (let i = 0; i < myThemeSlidersOff.length; i++) {
      myThemeSlidersOff[i].firstChild.setAttribute('d', sun);
    }
  }
